...
Inspired by OpenViking's progressive disclosure concept, cards support three loading levels. This stays pure markdown — no binary formats or external indexes required.

| Level | Source | Content | Token cost |
|-------|--------|---------|------------|
| L0 | `description` YAML field | One-line entity summary | ~10-20 tokens |
| L1 | `l1_summary` YAML field, or `## Summary` section, or first paragraph | 1-3 sentence summary | ~50-150 tokens |
| L2 | Full card body | Everything | Varies (see `content_tokens`) |

**How it works:**
- `compress` command auto-generates `l1_summary` and `content_tokens` in YAML frontmatter
- `generate-l1` regenerates L1 summaries from card content
- `search --level 0` returns only descriptions (cheapest, for bulk listing)
- `search --level 1` returns L1 summaries (good for triage and result previews)
- `search --level 2` (default) returns full card content

**When to use each level:**
- L0: Building indexes, counting cards, quick scans across hundreds of files
- L1: Search result previews, deciding which cards to read in full
- L2: Answering questions, updating cards, deep analysis
