...
## Hierarchical Loading (`--level`)

Control how much content is returned per card using `--level 0|1|2`:

| Level | What's returned | Token cost per card | Best for |
|-------|----------------|---------------------|----------|
| `--level 0` | `description` field only | ~10-20 | Bulk listing, building indexes, counting matches |
| `--level 1` | `l1_summary` (or `## Summary` / first paragraph fallback) | ~50-150 | Search result triage, deciding what to read in full |
| `--level 2` | Full card body (default) | Varies | Answering questions, deep analysis |

**Usage:**
```bash
# List all active cards with just descriptions
python3 memory-engine.py search "client" vault/ --mode normal --level 0

# Get summaries for triage
python3 memory-engine.py search "client" vault/ --mode normal --level 1

# Full content (default behavior)
python3 memory-engine.py search "client" vault/ --mode deep --level 2
```

**Tip:** Start with `--level 1` for broad searches, then read specific cards at L2. This keeps token costs low while still providing enough context for triage.

Use `--all` to include cards from all tiers regardless of search mode (equivalent to deep mode tier scope with any other mode's parameters).
