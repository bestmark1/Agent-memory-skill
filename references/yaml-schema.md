...
description: >-              # (required) One-line summary. What is this card about?
  FMCG client, NCP promo $66K, Pepsi mini-app in Uzbekistan
...
# ── ownership ──
owner: Shima                  # (optional) Who owns this relationship
responsible: Shima             # (optional) Who is doing the work
...
l1_summary: >-               # (auto) Cached L1 summary for hierarchical loading
  FMCG client in Uzbekistan, Pepsi mini-app, NCP promo $66K deal in negotiation
content_tokens: 342           # (auto) Cached token count of card body
---
```
...
Good: `"Event Specialist, Red Bull Kazakhstan, тендер 2026"`
...
### `l1_summary` (string, auto-generated)
Cached L1-level summary for hierarchical loading. Generated by `compress` or `generate-l1` commands. Contains a concise 1-3 sentence summary of the card content. If absent, the engine falls back to the `## Summary` section or first paragraph when L1 is requested.

### `content_tokens` (integer, auto-generated)
Cached approximate token count of the card body (excluding frontmatter). Updated by `compress` command. Used by `stats` to calculate context budget and emit warnings when limits are exceeded.
